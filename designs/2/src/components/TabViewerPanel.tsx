...
export function TabViewerPanel({ tabSourceUrl, currentTime, isPlaying }: TabViewerPanelProps) {
  ...
  if (!tabSourceUrl) {
    return (
      <div className="border px-4 py-3 font-mono text-sm" style={{ borderColor: "rgba(0, 255, 65, 0.1)", background: "rgba(10, 10, 10, 0.8)", color: "#3a3a3a" }}>
  ...
  return (
    <div className="border p-3" style={{ borderColor: "rgba(0, 255, 65, 0.12)", background: "rgba(10, 10, 10, 0.9)" }}>
      <h3 className="mb-2 font-mono text-xs font-medium uppercase tracking-widest" style={{ color: "#00e5ff" }}>// tab viewer</h3>
      <div ref={scrollHostRef} className="relative h-64 overflow-x-auto overflow-y-hidden rounded-sm bg-white p-3 text-black">
