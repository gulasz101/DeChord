      ...
      <nav className="relative z-10 flex shrink-0 items-center justify-between border-b px-6 py-3" style={{ borderColor: "rgba(192, 192, 192, 0.06)" }}>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => togglePanel("stems")}
            ...
            style={{ borderColor: sidePanel === "stems" ? "rgba(124, 58, 237, 0.4)" : "rgba(192, 192, 192, 0.12)", color: sidePanel === "stems" ? "#a78bfa" : "#c0c0c0" }}>
          ...
          <button onClick={() => togglePanel("comments")}
            className="flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition-colors hover:bg-white/5"
            style={{ borderColor: sidePanel === "comments" ? "rgba(124, 58, 237, 0.4)" : "rgba(192, 192, 192, 0.12)", color: sidePanel === "comments" ? "#a78bfa" : "#c0c0c0" }}>
        ...
        {sidePanel !== "none" && (
          <aside className="w-80 shrink-0 overflow-y-auto border-l p-4" style={{ borderColor: "rgba(192, 192, 192, 0.06)", background: "rgba(17, 22, 56, 0.7)", backdropFilter: "blur(16px)" }}>
            {sidePanel === "stems" && (
              <>
                <div className="mb-4 flex items-center justify-between">
                  <h3 className="text-sm font-semibold" style={{ fontFamily: "Orbitron, sans-serif", color: "#e2e2f0", fontSize: "0.7rem" }}>Stems</h3>
            ...
            {sidePanel === "comments" && (
              <>
                <div className="mb-4 flex items-center justify-between">
                  <h3 className="text-sm font-semibold" style={{ fontFamily: "Orbitron, sans-serif", color: "#e2e2f0", fontSize: "0.7rem" }}>Comments</h3>
                  <span className="text-[10px]" style={{ color: "#7a7a90" }}>{openCommentCount} open</span>
                </div>
                <div className="space-y-3">
                  {song.notes.map((note) => (
                    <div key={note.id} className="rounded-2xl border-l-2 border p-3" style={{ borderColor: "rgba(192, 192, 192, 0.04)", borderLeftColor: note.resolved ? "rgba(192, 192, 192, 0.1)" : "rgba(124, 58, 237, 0.5)", background: note.resolved ? "rgba(255, 255, 255, 0.01)" : "rgba(255, 255, 255, 0.02)", opacity: note.resolved ? 0.5 : 1 }}>
