  ...
  return (
    <div className="midnight-mesh min-h-screen" style={{ background: "linear-gradient(160deg, #0a0e27 0%, #111638 40%, #0a0e27 100%)" }}>
      ...
      <div className="relative z-10 mx-auto flex max-w-6xl gap-8 px-8 pt-8">
        ...
        <aside className="w-56 shrink-0 rounded-2xl border p-4" style={{ background: "rgba(255, 255, 255, 0.02)", borderColor: "rgba(192, 192, 192, 0.06)", backdropFilter: "blur(12px)" }}>
          <h3 className="mb-3 text-xs font-medium uppercase tracking-widest" style={{ fontFamily: "Orbitron, sans-serif", color: "#7a7a90", fontSize: "0.6rem" }}>Projects</h3>
          ...
          {/* Band members */}
          <h3 className="mb-3 mt-8 text-xs font-medium uppercase tracking-widest" style={{ fontFamily: "Orbitron, sans-serif", color: "#7a7a90", fontSize: "0.6rem" }}>Members</h3>
        ...
        <main className="flex-1">
          <div className="mb-6 flex items-end justify-between">
            <div>
              <h1 className="text-3xl" style={{ fontFamily: "Orbitron, sans-serif", color: "#e2e2f0" }}>{project.name}</h1>
              <p className="mt-1 text-sm" style={{ color: "#7a7a90" }}>{project.description}</p>
            </div>
            <button onClick={onOpenSongs} className="rounded-2xl px-6 py-2.5 text-sm font-semibold text-white transition-all hover:brightness-110 hover:shadow-purple-500/20" style={{ background: "linear-gradient(135deg, #7c3aed, #5b21b6)" }}>
              Song Library →
            </button>
          </div>
          ...
          <div className="mb-8 grid grid-cols-4 gap-3">
            {[
              ...
              <div key={s.label} className="rounded-xl border p-4" style={{ borderColor: "rgba(192, 192, 192, 0.06)", background: "rgba(255, 255, 255, 0.02)" }}>
                <div className="text-2xl font-bold" style={{ fontFamily: "Orbitron, sans-serif", color: "transparent", backgroundImage: "linear-gradient(135deg, #7c3aed, #14b8a6)", backgroundClip: "text", WebkitBackgroundClip: "text" }}>{s.value}</div>
          ...
          {/* Recent activity */}
          <h2 className="mb-4 text-lg" style={{ fontFamily: "Orbitron, sans-serif", color: "#e2e2f0", fontSize: "0.9rem" }}>Recent Activity</h2>
          <div className="space-y-2">
            {project.recentActivity.map((a) => (
              <div key={a.id} className="flex items-start gap-3 rounded-xl border-l-2 border p-4" style={{ borderColor: "rgba(192, 192, 192, 0.05)", borderLeftColor: "rgba(124, 58, 237, 0.4)", background: "rgba(255, 255, 255, 0.02)" }}>
          ...
          <h2 className="mb-4 mt-8 text-lg" style={{ fontFamily: "Orbitron, sans-serif", color: "#e2e2f0", fontSize: "0.9rem" }}>Songs Overview</h2>
          <div className="space-y-2">
            {project.songs.map((s) => {
              const sc = STATUS_COLORS[s.status] ?? STATUS_COLORS.uploaded;
              return (
                <div key={s.id} className="flex items-center gap-4 rounded-xl border p-4" style={{ borderColor: "rgba(192, 192, 192, 0.05)", background: "rgba(255, 255, 255, 0.02)" }}>
                  <div className="flex-1">
