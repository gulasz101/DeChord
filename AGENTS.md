<agents>
  <section id="branching-and-commits">
    ...
    <rule>Complete each task with a commit that references its plan path in docs/plans/, the specific task, the tool name (for example: opencode, claude-code, codex), and the model (for example: gpt-5.1-codex-max, opus-4.6, Qwen-Coder-Next, if model is not self aware, use the current CLI configuration to figure out which model is being used, perform this action using subagents).</rule>
    ...
    <rule>!ALWAYS! Delegate commit message curation and plan commit-link updates to a subagent subprocess to keep the main context lean.</rule>
  </section>
  ...
  <section id="skills-and-execution">
    <rule>ALWAYS use context-mode MCP to keep context as lean as possible!</rule>
  ...
  <section id="telegram">
    <when>ALWAYS when there is user input needed</when>
    <when>ALWAYS when the task is finished and summarty is presented</when>
