  ...
  it("renders loading and error states for stem upload", () => {
    render(
      <SongDetailPage
        user={user}
        band={band}
        ...
        onUploadStem={() => {}}
        uploadStemLoading
        uploadStemError="Stem upload failed"
      />,
    );

    fireEvent.click(screen.getByText("Upload Stem"));

    expect(screen.getByText("Saving stem...")).toBeTruthy();
    expect(screen.getByText("Stem upload failed")).toBeTruthy();
  });
});
